...
export const externalController = {
  ...
  async getDevFeed(_req: Request, res: Response) {
    const result = {
      hnStories: [] as { id: number; title: string; url: string; domain: string; score: number }[],
      trendingRepos: [] as { id: number; name: string; full_name: string; description: string; stargazers_count: number; html_url: string; language: string }[],
    };
    ...
    try {
      ...
      if (hnResponse.ok) {
        ...
        result.hnStories = stories
          .filter((s): s is PromiseFulfilledResult<any> => s.status === 'fulfilled' && s.value?.title)
          .map(s => ({
            id: s.value.id,
            title: s.value.title,
    ...
    try {
      ...
      if (ghResponse.ok) {
        ...
        result.trendingRepos = (json.items || []).slice(0, 10).map((r: any) => ({
          id: r.id,
          name: r.name,
