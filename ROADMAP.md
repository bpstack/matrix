...
- ✅ Si tiene tests (detectar carpetas test/, **tests**/, archivos _.test._, _.spec._)
...
### 5.2 Crypto Engine _(nuevo — paralelo con 5.1)_ ✅
...
### 5.3 Repository _(nuevo — depends on 5.1)_ ✅
...
### 5.4 Import TXT/CSV Parser _(nuevo — paralelo con 5.3)_ ✅
...
### 5.5 Controller _(nuevo — depends on 5.2, 5.3, 5.4)_ ✅
...
### 5.6 Routes _(nuevo — depends on 5.5)_ ✅
...
### 5.7 IPC para File Picker _(modifica archivos existentes)_ ✅
...
### 5.8 Hook _(nuevo — depends on 5.6)_ ✅
...
### 5.9 PasswordsView — 3 estados _(nuevo — depends on 5.8)_ ✅
...
### 5.10 Import Flow UI _(integrado en VaultView)_ ✅
...
### 5.12 i18n _(src/frontend/lib/i18n.ts)_ ✅
...
#### 6.3.1 Installer Builds (Windows + macOS + Linux) ✅

**Windows:**

- ✅ `authors` y `noMsi` añadidos a MakerSquirrel
- ✅ Build funcional: `out/make/squirrel.windows/x64/MatrixSetup.exe`
- ✅ Workflow CI/CD en `.github/workflows/release.yml`

**macOS:**

- ⏸ Diferido — requiere `icon.icns` y `@electron-forge/maker-dmg`. Hacer cuando se disponga de entorno macOS.

**Linux:**

- ✅ MakerDeb configurado

---

#### 6.3.2 Auto-updater (Electron) ✅

- ✅ Dependencia `update-electron-app` instalada
- ✅ Integración en `src/backend/index.ts`
- ✅ Solo activo si `app.isPackaged`
- ✅ Repo configurado: `bpstack/matrix`

---

#### 6.3.3 Tests con Vitest ✅

- ✅ `vitest.config.ts` configurado
- ✅ Helper `src/tests/helpers/db.ts` con seeders para todas las entidades
- ✅ Tests de **crypto engine** (`crypto.test.ts`): 16 tests
- ✅ Tests de **import-parser** (`import-parser.test.ts`): 17 tests
- ✅ Tests de **tasks controller** (`tasks.controller.test.ts`): 14 tests — vi.mock de repos, mockReq/mockRes helpers
- ✅ **Total: 47 tests pasando**

**Repository tests:** diferidos — requieren `better-sqlite3` recompilado para la versión de Node de Electron (necesita Python). Los controller tests con mocks cubren la lógica de negocio de forma equivalente.

---

#### 6.3.4 Error Handling + Logging centralizado ✅

- ✅ Módulo `src/backend/lib/logger.ts` (sin dependencias)
- ✅ Rotación: 5MB max, conserva últimas 2000 líneas
- ✅ `uncaughtException` + `unhandledRejection` capturados
- ✅ IPC handlers: `get-logs`, `clear-logs`, `get-log-path`
- ✅ Preload expone `window.matrix.getLogs/clearLogs/getLogPath`
- ✅ Toast store + ToastContainer en UI
- ✅ Visor de logs en SettingsView

---

#### 6.3.5 Keyboard Shortcuts (in-app) ✅

- ✅ Hook `useKeyboardShortcuts.ts` en AppShell
- ✅ Suprimido dentro de inputs (INPUT/TEXTAREA/SELECT)
- ✅ Shortcuts implementados:
  - `Ctrl+1..5`: tabs Overview/Projects/Tasks/Ideas/Passwords
  - `Ctrl+,`: Settings
  - `Ctrl+T`: Quick Task
  - `Ctrl+I`: Quick Idea
  - `Ctrl+B`: Toggle sidebar
- ✅ Referencia visual en SettingsView

---

#### 6.3.6 Alertas visuales de deadlines (in-app) ✅

- ✅ Hook `useDeadlines.ts` con polling cada 5 min
- ✅ Endpoint `/api/tasks/deadlines` en backend
- ✅ Clasificación: overdue / dueToday / dueSoon (7 días)
- ✅ Banner en header (rojo si overdue, amarillo si no)
- ✅ Badge rojo en tab Tasks del Sidebar
- ✅ Borde izquierdo coloreado en tarjetas de TaskBoard
